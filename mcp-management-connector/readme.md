...
- `script.csx` — Full MCP protocol implementation (initialize, notifications/initialized, tools/list, tools/call) proxying to Agent 365 MCP servers via `envId`
...
- Agent can call tools across Agent 365 MCP servers (AdminTools, SearchTools, Me, Dataverse, MCPManagement, Mail, Calendar, ODSPRemoteServer, SharePointListTools, Teams, Word)

## Notes
- Security: OAuth2 (Azure AD) using `https://agent365.svc.cloud.microsoft/.default` scope; headers are handled by the platform
- Protocol: JSON-RPC 2.0; methods like `initialize`, `notifications/initialized`, `tools/list`, `tools/call` are implemented
