...
- `discover_functions` — Discover available tools by intent keywords or category. Returns matching tools with relevance scores
- `invoke_tool` — Execute any tool dynamically by name with arguments. Bridges search results to execution
- `orchestrate_plan` — Run multi-step tool sequences with dependency handling. Supports variable substitution between steps
- `learn_patterns` — Retrieve learned patterns from organizational history. Filter by category, date range, or keyword
...
## Dynamic Instructions Table (AGENTS.md Pattern)

This connector supports dynamic, self-learning instructions stored in a Dataverse table. Create this table manually in your environment to enable adaptive agent behavior.

### Table Schema: `tst_agentinstructions`

| Column | Type | Description |
|--------|------|-------------|
| `tst_agentinstructionsid` | Uniqueidentifier | Primary key (auto-generated) |
| `tst_name` | Single Line Text (100) | **Primary Name / Alternate Key** - Instruction set identifier (e.g., "dataverse-tools-agent") |
| `tst_agentmd` | Multiline Text (100,000) | Main AGENTS.md content: tool definitions, selection guidance, parameter examples, org context |
| `tst_learnedpatterns` | Multiline Text (100,000) | Auto-populated by agent: successful query patterns and corrections |
| `tst_version` | Single Line Text (20) | Version tracking (e.g., "1.0", "2.1") |
| `tst_enabled` | Yes/No | Enable/disable this instruction set (default: Yes) |
| `tst_lastupdated` | Date/Time | Last modification timestamp |
| `tst_updatecount` | Whole Number | Count of learned pattern updates |

### Create Table Steps

1. Navigate to [Power Apps Maker Portal](https://make.powerapps.com)
2. Select your environment
3. **Tables** → **New table** → **New table**
4. Set display name: `Agent Instructions`
5. Set plural name: `Agent Instructions`
6. Set schema name prefix: `tst_`
7. Add columns per schema above
8. Set `cr_name` as **Alternate Key** for upsert operations
9. Save and publish

### Sample Record

Create a record with:
- **tst_name**: `dataverse-tools-agent`
- **tst_version**: `1.0`
- **tst_enabled**: `Yes`
- **tst_agentmd**: See [agents.md](agents.md) for content template
...
│ Query: tst_agentinstructions?$filter=tst_name eq 'dataverse-    │
│        tools-agent' and tst_enabled eq true&$top=1              │
└─────────────────────────────────────────────────────────────────┘
                              │
              ┌───────────────┴───────────────┐
              ▼                               ▼
┌─────────────────────────────┐     ┌─────────────────────────────────┐
│ Table/Record Found      │     │ Table/Record Not Found          │
│ → Use tst_agentmd       │     │ → Use default system prompt     │
│ → Append tst_learned    │     │                                 │
│   patterns              │     │                                 │
└─────────────────────────┘     └─────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│ Merge: BASE_PROMPT + DYNAMIC_INSTRUCTIONS + LEARNED_PATTERNS    │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│ After Successful Execution (Optional Self-Learning)             │
│ → Append successful pattern to tst_learnedpatterns              │
│ → Increment tst_updatecount                                     │
│ → Update tst_lastupdated                                        │
└─────────────────────────────────────────────────────────────────┘
```
...
| Feature | Description |
|---------|-------------|
| **Tool Selection Guidance** | Teach the agent which tools to prefer for specific intents |
| **Parameter Examples** | Provide correct OData syntax, table names, filter formats |
| **Organization Context** | Map business terms to Dataverse tables/columns |
| **Self-Learning** | Agent appends successful patterns for future reference |
| **Multi-Environment** | Different instructions per environment via tst_name |
| **Version Control** | Track instruction versions with tst_version |
| **Easy Updates** | Update instructions without redeploying connector |
...
└─────────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│ GetDynamicToolsAsync()                                          │
│ → Check _cachedTools (per-request cache)                        │
│ → If not cached, load from agents.md                            │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│ ParseToolsFromAgentMd()                                         │
│ → Find ## TOOLS section in agents.md content                    │
│ → Parse JSON array with tool definitions                        │
│ → Inject orchestration tools (search_tools, call_tool)         │
│ → Convert to MCP format (name, description, inputSchema)        │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│ Return tools to AI model for function calling                   │
└─────────────────────────────────────────────────────────────────┘
```

### Orchestration Tools

**discover_functions** - Discover relevant tools before execution
```json
{
...
```json
{
  ...
  "matchCount": 3,
  "tools": [
    { "name": "dataverse_create_row", "description": "...", "category": "WRITE", "score": 18 },
    { "name": "dataverse_upsert", "description": "...", "category": "WRITE", "score": 10 },
    { "name": "dataverse_create_multiple", "description": "...", "category": "BULK", "score": 8 }
  ]
}
```

**invoke_tool** - Execute tools dynamically
```json
{
  "name": "invoke_tool",
  "arguments": {
...
### Tool Categories
Tools in agents.md include category and keywords for search:
- **READ** - Query, retrieve, list operations
- **WRITE** - Create, update, delete operations
- **BULK** - Multi-record operations (CreateMultiple, UpdateMultiple)
- **RELATIONSHIPS** - Associate, disassociate records
- **METADATA** - Entity, attribute, relationship discovery
- **SECURITY** - Roles, sharing, access control
- **RECORD_MGMT** - State, assignment, merge operations
- **ATTACHMENTS** - File upload/download
- **CHANGE_TRACKING** - Delta sync operations
- **ASYNC** - Background job management
- **ADVANCED** - Actions, functions, batch operations
...
**MCP Tools**: 49 (45 Dataverse + 4 orchestration tools)  
**Orchestration Tools**: search_tools, call_tool, execute_workflow, get_patterns  
**Dynamic Loading**: Tools and instructions loaded from `tst_agentinstructions` table at runtime
