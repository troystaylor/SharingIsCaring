...
if (-not (Test-Path $manifestPath)) {
    ...
    if ($manifest.agentConnectors) {
        ...
        foreach ($conn in $manifest.agentConnectors) {
            ...
            if ($conn.toolSource) {
                $hasPluginSource = $null -ne $conn.toolSource.plugin
                $hasRemoteSource = $null -ne $conn.toolSource.remoteMcpServer

                if (($hasPluginSource -and $hasRemoteSource) -or (-not $hasPluginSource -and -not $hasRemoteSource)) {
                    $errors += "Connector '$($conn.id)': toolSource must contain exactly one of plugin or remoteMcpServer"
                }

                if ($hasRemoteSource) {
                    $url = [string]$conn.toolSource.remoteMcpServer.mcpServerUrl
                    if ([string]::IsNullOrWhiteSpace($url)) {
                        $errors += "Connector '$($conn.id)': remoteMcpServer.mcpServerUrl is required"
                    }
                    elseif ($url -notmatch '^https://') {
                        $errors += "Connector '$($conn.id)': mcpServerUrl must be HTTPS"
                    }
                    elseif (-not [Uri]::IsWellFormedUriString($url, [UriKind]::Absolute)) {
                        $errors += "Connector '$($conn.id)': mcpServerUrl must be a valid absolute URL"
                    }
                    ...
                    if ($null -eq $auth -or [string]::IsNullOrWhiteSpace([string]$auth.type)) {
                        $errors += "Connector '$($conn.id)': authorization.type is required"
                    }
                    else {
                        $authType = [string]$auth.type
                        $hasReferenceId = -not [string]::IsNullOrWhiteSpace([string]$auth.referenceId)

                        if ($authType -eq "None" -and $hasReferenceId) {
                            $errors += "Connector '$($conn.id)': authorization.referenceId must NOT be present when type is None"
                        }
                        if ($authType -ne "None" -and -not $hasReferenceId) {
                            $errors += "Connector '$($conn.id)': authorization.referenceId is required unless type is None"
                        }
                    }
                }
            }
...
if ($manifest.agentSkills) {
    foreach ($skillEntry in $manifest.agentSkills) {
        ...
        foreach ($file in $companionFiles) {
            ...
            $totalSize += $sizeBytes
            $relativePath = Get-RelativePathCompat -BasePath $skillDir -TargetPath $file.FullName
            $normalizedRelativePath = $relativePath -replace '\\', '/'
            $pathSegments = $normalizedRelativePath.Split('/', [System.StringSplitOptions]::RemoveEmptyEntries)
            ...
            if ([System.IO.Path]::IsPathRooted($relativePath)) {
                $errors += "'$folder/$($file.Name)': path must be relative (no absolute paths)"
            }

            if ($normalizedRelativePath -match '(^|/)\.\.(/|$)') {
                $errors += "'$folder/$($file.Name)': path traversal (..) is not allowed"
            }

            if ($normalizedRelativePath -match "`0") {
                $errors += "'$folder/$($file.Name)': null byte is not allowed"
            }

            foreach ($segment in $pathSegments) {
                if ($segment.StartsWith('.')) {
                    $errors += "'$folder/$normalizedRelativePath': hidden files or folders are not allowed"
                    break
                }

                $segmentBaseName = [System.IO.Path]::GetFileNameWithoutExtension($segment)
                if ($segmentBaseName -imatch '^(CON|PRN|AUX|NUL|COM[1-9]|LPT[1-9])$') {
                    $errors += "'$folder/$normalizedRelativePath': contains Windows reserved name '$segment'"
                    break
                }

                if ($segment -notmatch '^[A-Za-z0-9 _.!-]+$') {
                    $errors += "'$folder/$normalizedRelativePath': contains unsafe characters"
                    break
                }
            }
        }
...
if (Test-Path (Join-Path $pluginPath "color.png")) { $items += "color.png" }
if (Test-Path (Join-Path $pluginPath "outline.png")) { $items += "outline.png" }
if (Test-Path (Join-Path $pluginPath "mcp-tool-description.md")) { $items += "mcp-tool-description.md" }
if (Test-Path (Join-Path $pluginPath "mcp-tool-description.json")) { $items += "mcp-tool-description.json" }
if (Test-Path (Join-Path $pluginPath "skills")) { $items += "skills" }
