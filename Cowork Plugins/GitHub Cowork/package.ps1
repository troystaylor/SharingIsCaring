    ...
    if ($manifest.agentConnectors) {
        ...
        foreach ($conn in $manifest.agentConnectors) {
            ...
            if ($conn.toolSource.remoteMcpServer) {
                ...
                # mcpToolDescription is required on every remoteMcpServer connector.
                # Omitting it causes the package service to reject the upload with HTTP 400.
                $toolDesc = $conn.toolSource.remoteMcpServer.mcpToolDescription
                if (-not $toolDesc -or -not $toolDesc.file) {
                    $errors += "Connector '$($conn.id)': mcpToolDescription.file is required on each remoteMcpServer"
                }
                else {
                    # The package service resolves this path literally - a './' prefix
                    # fails with "not found in the app package".
                    if ($toolDesc.file -match '^\.[\\/]') {
                        $errors += "Connector '$($conn.id)': mcpToolDescription.file must not start with './' - use a bare relative path like 'tools/my-tools.json'"
                    }
                    if ($toolDesc.file -match '\\') {
                        $errors += "Connector '$($conn.id)': mcpToolDescription.file must use forward slashes"
                    }

                    $toolFile = Join-Path $pluginPath ($toolDesc.file -replace '^\./', '')
                    if (-not (Test-Path $toolFile)) {
                        $errors += "Connector '$($conn.id)': mcpToolDescription file not found in package: $($toolDesc.file)"
                    }
                    else {
                        try {
                            $td = Get-Content $toolFile -Raw | ConvertFrom-Json
                            if (-not $td.tools) {
                                $errors += "Connector '$($conn.id)': $($toolDesc.file) must contain a 'tools' array"
                            }
                            else {
                                foreach ($t in $td.tools) {
                                    if (-not $t.name) {
                                        $errors += "Connector '$($conn.id)': a tool in $($toolDesc.file) is missing 'name'"
                                        continue
                                    }
                                    if (-not $t.description) { $warnings += "Connector '$($conn.id)': tool '$($t.name)' has no description" }
                                    # Per the MCP spec the package service requires inputSchema
                                    # as a JSON Schema object on every tool.
                                    if (-not $t.inputSchema) {
                                        $errors += "Connector '$($conn.id)': tool '$($t.name)' is missing 'inputSchema' (required by the MCP spec)"
                                    }
                                    elseif ($t.inputSchema.type -ne 'object') {
                                        $errors += "Connector '$($conn.id)': tool '$($t.name)' has an inputSchema whose 'type' is not 'object'"
                                    }
                                }
                            }
                        }
                        catch {
                            $errors += "Connector '$($conn.id)': $($toolDesc.file) is not valid JSON"
                        }
                    }
                }
                ...
                if ($auth) {
                    ...
                    if ($auth.referenceId -and $auth.referenceId -match 'REPLACE_WITH|\{\{|<YOUR') {
                        $warnings += "Connector '$($conn.id)': referenceId is still a placeholder - the connector will not authenticate until it is set"
                    }
                }
            }
        }
...
if ($Json) {
    ...
    if ($errors.Count -eq 0) {
        ...
        if (Test-Path (Join-Path $pluginPath "tools")) { $items += "tools" }
