...
internal static class SearchCustomersTool
{
    public static ToolDescriptor Build() => new()
    {
        Name = "search_customers",
        Description = "Search NetSuite customers by company name or email. Returns id, company name, email, phone, and inactive flag. Wraps a SuiteQL query against the customer table.",
        Annotations = new ToolAnnotations(
            Title: "Search NetSuite customers",
            ReadOnlyHint: true,
            OpenWorldHint: true),
        InputSchema = Schema(
            new JsonObject
            {
                ...
                ["includeInactive"] = Prop("boolean", "Include inactive customers (default false)."),
