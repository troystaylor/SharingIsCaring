<#
.SYNOPSIS
    Preflight checks for NetSuite Cowork plugin cutover.
...
if (-not (Test-Path $manifestPath)) {
    ...
    if ($connector) {
        ...
        elseif ($mcpUrl -match 'replace-with|your-netsuite-mcp-endpoint|YOUR-CONTAINER-APP-FQDN|\{\{') {
        ...
        elseif ($referenceId -match 'replace-with|registration-id|OAUTH_REFERENCE_ID|\{\{') {
...
Write-Host "`nNetSuite preflight results" -ForegroundColor Cyan
Write-Host ("=" * 40)
