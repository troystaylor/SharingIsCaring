...
Upload files from any public HTTPS URL directly into SharePoint document libraries using Copilot Cowork. Small files upload inline; large files (250 MB+) stream in the background with resumable chunked uploads. Browse sites, libraries, and folders. Create folders, move/rename items, set metadata columns, and generate sharing links.
