...
namespace SalesforceCoworkMcp.Endpoints;
...
public static class McpEndpoints
{
        ...
        IReadOnlySet<string>? filter)
    {
        app.MapPost(path, async (HttpContext ctx) =>
        {
            ...
            // One-line diagnostic per request — never logs the token itself.
            ...
            // If any handler raised an auth challenge, surface it via HTTP so the MCP client
            // (Cowork) knows to re-trigger OAuth instead of just showing a tool error.
        ...
        IReadOnlySet<string>? filter)
    {
        ...
        catch (UnauthorizedSalesforceException ex)
        {
            // Record on context so MapPost can emit HTTP 401 + WWW-Authenticate.
            ctx.Items[AuthChallengeKey] = new AuthChallenge(ex.ErrorCode, ex.Message);
            return Err(id, -32001, ex.Message, new JsonObject
            {
                ["error_code"] = ex.ErrorCode,
                ["action"] = "reauthorize_salesforce_connector",
            });
        }
