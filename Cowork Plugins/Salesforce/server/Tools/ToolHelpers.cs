...
internal static class ToolHelpers
{
    ...
    public static decimal? OptDecimal(JsonObject args, string name)
    {
        if (args[name] is JsonValue v)
        {
            if (v.TryGetValue<decimal>(out var d)) return d;
            if (v.TryGetValue<double>(out var dd)) return (decimal)dd;
            if (v.TryGetValue<string>(out var s) && decimal.TryParse(s, out var p)) return p;
        }
        return null;
    }
