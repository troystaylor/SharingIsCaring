...
public class ConversationMappingStore
{
    ...
    {
        // Store bidirectional mapping (wrap in dictionary for MemoryStorage compatibility)
        var data = new Dictionary<string, object>
        {
            [$"{SnToMcsKeyPrefix}{snConversationId}"] = new Dictionary<string, string> { ["value"] = mcsConversationId },
            [$"{McsToSnKeyPrefix}{mcsConversationId}"] = new Dictionary<string, string> { ["value"] = snConversationId }
        };
    ...
    public async Task<string?> GetMcsConversationIdAsync(string snConversationId)
    {
        ...
        {
            if (val is IDictionary<string, object> dict && dict.TryGetValue("value", out var v))
                return v?.ToString();
            if (val is System.Text.Json.JsonElement je && je.TryGetProperty("value", out var jp))
                return jp.GetString();
        }
        return null;
    }
    ...
    private async Task UpdateRegistryAsync(string snConversationId, bool add)
    {
        ...
        try
        {
            ...
            // Wrap in dictionary for MemoryStorage compatibility
            await _storage.WriteAsync(new Dictionary<string, object>
            {
                [RegistryKey] = new Dictionary<string, string> { ["value"] = JsonSerializer.Serialize(activeIds) }
            });
        }
        finally
        {
    ...
    private async Task<List<string>> ReadRegistryAsync()
    {
        ...
        if (data.TryGetValue(RegistryKey, out var val))
        {
            ...
            if (val is IDictionary<string, object> dict && dict.TryGetValue("value", out var v))
                json = v?.ToString();
            else if (val is System.Text.Json.JsonElement je && je.TryGetProperty("value", out var jp))
                json = jp.GetString();
