...
// Storage — MemoryStorage for dev only. For production, use:
//   Azure Blob Storage: Microsoft.Agents.Storage.Blobs
//   Cosmos DB: Microsoft.Agents.Storage.CosmosDb
// MemoryStorage loses all state on restart and doesn't work with multiple instances.
builder.Services.AddSingleton<IStorage, MemoryStorage>();
builder.Services.AddSingleton<AgentApplicationOptions>();

// Copilot Studio client factory
...
static void ValidateConfiguration(IConfiguration config)
{
    ...
    // Copilot Studio — need at least DirectConnectUrl or (EnvironmentId + SchemaName)
    var cps = config.GetSection("CopilotStudioClientSettings");
    if (string.IsNullOrEmpty(cps["DirectConnectUrl"]) && string.IsNullOrEmpty(cps["EnvironmentId"]))
        warnings.Add("CopilotStudioClientSettings: DirectConnectUrl or EnvironmentId is required. " +
            "Get the connection string from Copilot Studio > Channels > Web app > Copy connection string.");
