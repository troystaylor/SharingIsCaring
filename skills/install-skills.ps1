# Install a skills repo globally or to workspace
param(
...
if ($Workspace) {
    # Workspace mode: install to .github/skills/ in current directory
    ...
    Write-Host "Installing to workspace: $SkillsRoot" -ForegroundColor Cyan
    ...
    # Copy skills from nested path to .github/skills/ root for auto-discovery
    if ($SkillsSubPath -and (Test-Path $NestedSkillsPath)) {
        $SkillFolders = Get-ChildItem -Path $NestedSkillsPath -Directory
        foreach ($Folder in $SkillFolders) {
            $Dest = Join-Path $SkillsRoot $Folder.Name
            if (-not (Test-Path $Dest)) {
                Copy-Item -Path $Folder.FullName -Destination $Dest -Recurse
                Write-Host "Copied skill: $($Folder.Name)" -ForegroundColor Green
            } else {
                Write-Host "Skill already exists: $($Folder.Name)" -ForegroundColor Yellow
            }
        }
        # Optionally remove the cloned repo after copying
        Remove-Item -Path $ClonePath -Recurse -Force
        Write-Host "Cleaned up clone directory" -ForegroundColor Gray
    }
    
    Write-Host "
Done! Skills installed to .github/skills/ - auto-discovered by VS Code." -ForegroundColor Green
    
} else {
    # Global mode: install to user profile and update settings
